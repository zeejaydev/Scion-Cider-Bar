...
const items = ()=>{
    fetch('https://scionciderbar.herokuapp.com/items').then((res)=>res.json()).then(async(resp)=>{
        ...
        const modifiers = await fetch('https://scionciderbar.herokuapp.com/modi').then((res)=>res.json()).then(resp=>resp.objects.filter((m)=>m.modifierData.modifierListId==="5AYQUTHAJEOSUMRZNDFSNQ5R")).catch(e=>console.log(e));
        const modifiers2 = await fetch('https://scionciderbar.herokuapp.com/modi').then((res)=>res.json()).then(resp=>resp.objects.filter((m)=>m.modifierData.modifierListId==="J2A4U7DXTQBRJIMYOCRK4KIC")).catch(e=>console.log(e));

          const arrangedCats = cats.filter((items)=>items.id !=='7RAPLTQPXVWON2HUBSGG4FNH'?items:null).map((item,index) => {
              if(item.id==='JSZPGWW57OER7KURJAB5CEC7')return({value:item,sort:0})
              ...
              else{return{value:item,sort:index+2}}
                ...
                if(i.itemData.categoryId===item.id){
                    ...
                    <div class='menu-content'>
                        <div class="content-name">
                            ...
                            ${ item.id==="D56LQML6UIEZRD7PCFS4WSSI"?
                                `
                                    
                                        ${modifiers.map((m)=>{
                                            return `
                                                <div class='modifiers'>
                                                    <h3>${m.modifierData.name}</h3> 
                                                    <span class='col'>${parseInt(m.modifierData.priceMoney.amount.replace('n', ''))/100+parseInt(money.replace('n', ''))/100}</span>
                                                </div>
                                            `
                                        }).join('')
                                        }
                            ...
                            ${ i.id==="KWC3UR2ETM7PM4RYN333TEWT"?
                                `
                                   
                                   
                                        ${modifiers2.map((m)=>{
                                            const price = parseInt(m.modifierData.priceMoney.amount.replace('n', ''))/100
                                            return `
                                               
                                                <h3 class='modifiers'>${m.modifierData.name} <span>${price===0?'':price}</span></h3> 
                                            `
                                        }).join('')
                                        }
                        ...
                        ${  /*specifiying a cat without origin abv taster etc cols*/
                            item.id==='JSZPGWW57OER7KURJAB5CEC7'?''
                        :
                            `
                            <div class="content-cols">
                                ...
                                ${money===undefined||item.id==='D56LQML6UIEZRD7PCFS4WSSI'?'':`<div class="col">${parseInt(money.replace('n', ''))/100}</div>`} 
        ...
        const cider = arrangedCats.map((i)=>{
            if(i.id==='DILT6SMUJB5CACU5PMFRVXBQ' 
                || i.id==='PNH5WL2YZRSEAWZAAOZ7N3C2' 
                || i.id==='QTYTCB4UFRVI6FDRDONE46B6'
                || i.id==='4FRYW7EGFV36224NZM6P2O2F'
                || i.id==='FWX2W6RP2DQR6NLT4OJEFEYK'
                || i.id==='IND4F6VXIWFYWFQHAO2Q7KH5'
                ){
                    return`
                       <a href="#${i.categoryData.name}" >${i.categoryData.name}</a>  
                   `
        ...
        const beer = arrangedCats.map((i)=>{
            if(i.id==='BJTCCFR7Q7LF7OHVTTHYW2HW' 
                || i.id==='XZURZMLETKUSM6JCUEUEP2JE' 
                ){
                    return`
                       <a href="#${i.categoryData.name}" >${i.categoryData.name}</a>  
                   `
        ...
        const wine = arrangedCats.map((i)=>{
            if(i.id==='LLPRMGLDMD25JC37P5SC3BA7' 
                || i.id==='A7STWVI3EH4M23ZOH6DGOU43' 
                ){
                    return`
                       <a href="#${i.categoryData.name}" >${i.categoryData.name}</a>  
                   `
        ...
        const spirits = arrangedCats.map((i)=>{
            if(i.id==='HIBXHBJSI4LHQJ4ZE43OIQM5' 
                || i.id==='GB62Z26FH23LDR4PUYUBYZFL' 
                ){
                    return`
                       <a href="#${i.categoryData.name}" >${i.categoryData.name}</a>  
                   `
        ...
        document.getElementById("ciderItems").insertAdjacentHTML("beforeend", cider);
        document.getElementById("beerItems").insertAdjacentHTML("beforeend", beer);
        document.getElementById("wineItems").insertAdjacentHTML("beforeend", wine);
        document.getElementById("spiritItems").insertAdjacentHTML("afterbegin", spirits);
